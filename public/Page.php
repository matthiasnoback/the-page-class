...
class Page
{
    ...
    public function __construct($uri)
    {
        ...
        if (trusted_ip()) {
            ...
            // TODO: we'd like to use E_ALL here ;)
            error_reporting(
...
//        }
    ...
    {
        $result = mysql_query(
            "SELECT c.id, c.uri, c.title, c.menu_name, c.node, c.skip_to_first_subpage FROM content c WHERE c.id='$id';"
        );
    ...
    function determine_page($url)
    {
        ...
        foreach ($url_parts as $key => $part) {
            ...
            $result = mysql_query("SELECT id, skip_to_first_subpage " .
                "FROM content WHERE uri='" . addslashes($part) . "' " .
        ...
        while (empty($page_ids)
            ...
            $page_id = $this->find_first_subpage(
                (int)$page_ids[count($page_ids) - 1]['id']);
            $result = mysql_query("SELECT id, skip_to_first_subpage " .
    ...
    function open_page()
    {
        $result = mysql_query("SELECT * FROM content c WHERE id='{$this->page['id']}';");
        if ($result && mysql_num_rows($result)) {
...
//        $timer->startTimer();
        ...
        $menu_items = array();
        $result = mysql_query("SELECT c.id, c.uri, c.menu_name " .
            "FROM content c " .
            "WHERE " .
...
//    $timer->addTime();
    ...
    function subnavigation()
    {
        if ($this->page['id'] != $this->parent_node
            ...
            if ($this->page['parent_id'] != $this->parent_node
...
//        $timer->addTime();
    ...
    public function connect_db()
    {
        ...
        if ($this->db_connection) {
            ...
            if (!$this->db) {
                ?><p class="warning">Geen database.</p><?php
                exit;
            }
        } else {
            ?><p class="warning">Geen verbinding.</p><?php
            exit;
        }
    }
